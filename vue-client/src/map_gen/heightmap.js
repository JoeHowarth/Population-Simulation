...
async function genHM(mesh) {
    ...
    // @ts-ignore
    let h = add(slope(mesh, randDir(), 0.5), 
    ...
    cone(mesh, 0.0015, [rnorm(W / 2, W / 3), rnorm(H / 2, H / 3)]), mountains(mesh, shNum, 5, 0.7), mountains(mesh, shNum, 5, 0.7), mountains(mesh, rhNum, 8, 0.4), mountains(mesh, mtNum * 0.5, 30, 0.4), mountains(mesh, mtNum * 0.5, 15, 0.8), mountains(mesh, mtNum * 0.2, 35, 0.1));
...
/*
function findSinks(mesh: Mesh, h: MapData) {
  let dh = downhill(mesh, h);
...
*/
function fillSinks(mesh, h, epsilon = 1e-5) {
    let infinity = 999999;
    let newh = mesh.zero()
        .map((v, i) => {
...
export { genHM, fillSinks, doErosion, erode, erosionRate, getFlux, normalize, peaky, getSlope, cone, downhill, mountains, quantile, setSeaLevel, cleanCoast, getRivers, placeCities, cityScore, };
