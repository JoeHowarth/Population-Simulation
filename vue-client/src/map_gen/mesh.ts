...
async function makeMesh(vor, ctx, [Wkm, Hkm]: [number, number], [Wpx, Hpx]: [number, number]): Promise<Mesh> {
...
export function mesh_from_data(mesh: Mesh): Mesh {
...
function calcArea(mesh: Mesh): MapData {
  let area = []
  for (let i = 0; i < mesh.centroids.length; i++) {
    let points = mesh.triPaths[i]
    area.push(areaTriangle(<any>points))
  }
  mesh.area = area
  console.log(area)
  return area
...
function areaTriangle([[x1, y1], [x2, y2], [x3, y3]]: [Point, Point, Point]): number {
  return -(0.5 * (x2 - x1) * (y3 - y1) - 0.5 * (x3 - x1) * (y2 - y1))
}

/// re-index based off triIDs instead of original
function calcTriIDs(mesh: Mesh): (Uint32Array | Map<any, any>)[] {
  console.time('goodTris')
  let goodTris: number[] = []
  Array.from(mesh.delaunay.trianglePolygons())
      .forEach((pts, i) => {
        const [a, b, c]: [Point, Point, Point] = <any>pts
...
function calcAdj(mesh: Mesh) {
  const {halfedges, invTriIDs} = mesh
  let adj: number[][] = []
  console.time('adj')
  for (let i = 0; i < halfedges.length; i++) {
    let e0 = i
    let t0_ = (<Map<any,any>>invTriIDs).get(edge2tri(i))
    if (t0_ === undefined) continue
    
    let e1 = halfedges[e0]
    if (e1 === -1) continue
    
    let t1_ = (<Map<any,any>>invTriIDs).get(edge2tri(e1))
...
export function calcAdjVerts(mesh: Mesh) {
  const {halfedges, invTriIDs} = mesh
  let adj: number[][] = []
  for (let i = 0; i < halfedges.length; i++) {
    let e0 = i
...
function triSlope(mesh: Mesh): (h: MapData, i: number) => Point {
  return (h, i) => {
    let nbs = mesh.adj[i]
...
function calcNormPts(mesh: Mesh): Point[] {
  let xs: number[] = []
  let ys: number[] = []
...
export function lengthSquared([ax, ay]: number[], [bx, by]: number[]) {
  let dx = ax - bx
  let dy = ay - by
  return dx * dx + dy * dy
}
