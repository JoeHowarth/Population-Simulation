...
async function genHM(mesh: Mesh) {
...
function relax(mesh: Mesh, h: MapData) {
  let newh = mesh.zero()
...
function mountains(mesh: Mesh, n: number, r: number, h: number): MapData {
  r = r || 100;
  h = h || 1;
  let mounts: Array<Point> = [];
...
function doErosion(mesh: Mesh, h: MapData, amount: number, n: number): MapData {
  n = n || 1;
  h = fillSinks(mesh, h);
  for (let i = 0; i < n; i++) {
...
function fillSinks(mesh: Mesh, h: MapData, epsilon = 1e-5) {
  let infinity = 999999;
  let newh = mesh.zero()
    .map((v, i) => {
